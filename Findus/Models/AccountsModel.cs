...
//TODO: Rename VATAccounts & other Vat... to PurchaseAccounts & Purchase...
...
namespace Findus.Models
{
    ...
    public class AccountsModel
    {
        public AccountsModel(
            Dictionary<string, AccountModel> vat,
            Dictionary<string, AccountModel> sales
        )
        {
            VAT = vat;
            Sales = sales;
        ...
        public RateModel GetPurchaseAccount(WcOrder order, OrderLineItem item)
        {
            var acc = GetVATAccount(order);
        ...
        public RateModel GetSalesAccount(WcOrder order, OrderLineItem item)
        {
            var acc = GetSalesAccount(order);
            if (item.HasReducedRate())
            {
                return acc.Reduced;
            }
        ...
        private static AccountModel GetAccount(
            Dictionary<string, AccountModel> accounts,
            string countryIso,
        ...
        )
        {
            ...
            else if (string.IsNullOrEmpty(payment_method))
            {
                throw new ArgumentException(
                    "Payment Method required for countries outside EU: Expected Stripe or PayPal. Maybe this order should be processed manually?"
                );
            }
            return accounts[payment_method];
        }

        public RateModel GetSales(
            string countryIso,
            bool isStandard = false,
            bool isReduced = false
        )
        {
            if (isStandard && isReduced)
                throw new Exception("Account rate can only be either standard or reduced at once.");
            if (!Sales.ContainsKey(countryIso))
                countryIso = "NON_EU";
